...
<template>
    <div class="contract-menu" style="position: relative;height: 100% ;">
        <div class="contract-menu-header">
            <el-tooltip class="item" effect="dark" :content="$t('message.contractIDE.addFile')" v-if="!disabled" placement="top-start">
                <i class="wbs-icon-Addfile icon contract-icon" @click='addFile'></i>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" v-if="!disabled" :content="$t('message.contractIDE.addFolder')" placement="top-start">
                <i class="wbs-icon-Addfolder icon contract-icon" @click='addFolder'></i>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" :content="$t('message.contractIDE.uploadFile')" v-if="!disabled" placement="top-start">
            ...
            <el-tooltip effect="dark" :content="$t('message.contractIDE.fileTips')" placement="top-start">
            ...
            <ul>
                <li v-for='item in contractArry' :key="item.contractId">
                    <div v-if='item.contractType == "file"' class="contract-file" :id='item.contractId'>
                        ...
                        <el-input size="mini" v-model="contractName" v-focus="true" autofocus='autofocus' maxlength="32" @blur="changeName(item)" v-if="item.renameShow"></el-input>
                        ...
                        <div class="contract-menu-handle" v-if='!disabled &&item.handleModel' :style="{'top': clentY,'left': clentX}" v-Clickoutside="checkNull">
                            <ul v-if="contractFile">
                                <li class="contract-menu-handle-list" @click="rename">{{$t('message.contractIDE.rename')}}</li>
                                <li class="contract-menu-handle-list" v-if='!item.renameShow && !item.contractAddress' @click="deleteFile(item)">{{$t('message.common.delete')}}</li>
                            </ul>
                            <!-- <ul v-if="contractFolder">
                                <li class="contract-menu-handle-list" @click="addFiles">{{$t('message.contractIDE.addFile')}}</li>
                            </ul> -->
                        </div>
                    </div>
                    <div v-if='item.contractType == "folder"' class="contract-folder" :id='item.folderId'>
                        ...
                        <!-- <el-input size="mini" v-model="contractName" autofocus='autofocus'  @blur="changeName(item)" v-if="item.renameShow"></el-input> -->
                        <!-- <i class="wbs-icon-delete contract-delete" v-if="!item.renameShow" @click='deleteFolder(item)'></i> -->
                        <div class="contract-menu-handle" v-if='!disabled && item.handleModel' :style="{'top': clentY,'left': clentX}" v-Clickoutside="checkNull">
                            <ul>
                                <li class="contract-menu-handle-list" @click="addFiles">{{$t('message.contractIDE.addFile')}}</li>
                                <li class="contract-menu-handle-list" v-if="!item.renameShow" @click='deleteFolder(item)'>{{$t('message.common.delete')}}</li>
                            </ul>
                        </div>
                        <br>
                        <ul v-if="item.folderActive" style="padding-left: 20px;">
                            <li class="contract-file" v-for='list in item.child' :key="list.contractId">
                                ...
                                <el-input size="mini" v-model="contractName" autofocus='autofocus' maxlength="32" @blur="changeName(list)" v-if="list.renameShow"></el-input>
                                ...
                                <div class="contract-menu-handle" v-if='!disabled &&list.handleModel' :style="{'top': clentY,'left': clentX}" v-Clickoutside="checkNull">
                                    <ul v-if="contractFile">
                                        <li class="contract-menu-handle-list" @click="rename">{{$t('message.contractIDE.rename')}}</li>
                                        <li class="contract-menu-handle-list" v-if='!list.renameShow && !list.contractAddress' @click="deleteFile(list)">{{$t('message.common.delete')}}</li>
                                    </ul>
                                </div>
                            </li>
                        </ul>
...
export default {
    ...
    mounted: function () {
        ...
        Bus.$on("compile", data => {
            this.saveContract(data, this.$i18n.t('message.contractIDE.conBuildSuc'))
        })
        Bus.$on("save", data => {
    ...
    methods: {
                ...
                if (value.contractId == this.ID && value.contractStatus != 2) {
                    ...
                    value.child.forEach(item => {
                        if (item.contractId == this.ID && item.contractStatus != 2) {
                            ...
                            this.$message({
                                message: this.$i18n.t('message.contractIDE.conNoReName'),
                                type: "error"
                            });
                        } else {
                            this.$set(item, 'renameShow', false)
        ...
        changeName: function (val) {
            ...
            if (pattern.test(this.contractName) && this.contractName.length < 32 && this.contractName.length > 1) {
                if (this.contractName !== val.contractName) {
                    for (let i = 0; i < this.contractList.length; i++) {
                        if (this.contractList[i].contractName == this.contractName && this.contractList[i].contractPath == val.contractPath) {
                            this.$message({
                                message: this.$i18n.t('message.contractIDE.repeatName'),
                                type: "error"
                            });
                ...
                this.$message({
                    message: this.$i18n.t('message.contractIDE.number1to32'),
                    type: "error"
                });
                this.$set(val, 'renameShow', false)
            }
        ...
        upload: function (e) {
            ...
            for (let i = 0; i < this.uploadFiles.length; i++) {
                ...
                if (filessize > 400) {
                    this.$message({
                        message: this.$i18n.t('message.contractIDE.fileSize400'),
                        type: "error"
                    });
                    ...
                    this.$message({
                        message: this.$i18n.t('message.contractIDE.uploadSol'),
                        type: "error"
                    });
                    this.cataLogShow = false
                    break;
            ...
            for (let i = 0; i < this.uploadFiles.length; i++) {
                ...
                this.contractList.forEach(value => {
                    if (value.contractName == filename && value.contractPath == val&& num===0) {
                        this.$message({
                            type: "error",
                            message: this.$i18n.t('message.contractIDE.sameConName')
                        });
                        num++;
                    }
                });
            ...
            saveChaincode(reqData).then(res => {
                if (res.data.code === 0) {
                    ...
                    if (data.contractId) {
                        this.$message({
                            type: "success",
                            message: title || this.$i18n.t('message.contractIDE.conSaveSuc')
                        });
                    }
                } else {
                    this.$message({
        ...
        deleteFile: function (val) {
            this.$confirm(this.$i18n.t('message.common.confirmDelete'),'', {
        ...
        deleteFolderData: function (val) {
            ...
            for (let i = 0; i < list.length; i++) {
                if (list[i].contractStatus == 2) {
                    this.$message({
                        type: "error",
                        message: this.$i18n.t('message.contractIDE.notDelete')
                    })
                    return
                } else {
                    num++
