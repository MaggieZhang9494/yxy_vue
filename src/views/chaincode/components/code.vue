...
<template>
    <div class="contract-code" :class="{changeActive:changeWidth }" v-loading="loading">
        ...
        <div class="contract-code-content" :class="{infoHide: !successHide}">
            <div class="contract-code-mirror" :style="{height:codeHight}" ref="codeContent">
                <div style="padding-top: 60px;text-align:center;" v-show="!codeShow">
                    <span>{{$t('message.contractIDE.openCon')}}</span>
                </div>
                ...
                <div>
                    ...
                    <div class="contract-info-list" v-show="contractAddress">
                        <span class="contract-info-list-title" style="color: #0B8AEE">contractAddress 
                            <i class="wbs-icon-copy font-12 copy-public-key" @click="copyKey(contractAddress)" :title="$t('message.common.copy')"></i>
                        </span>
                    ...
                    <div class="contract-info-list" v-show="abiFile">
                        <span class="contract-info-list-title" style="color: #0B8AEE">contractName
                            <i class="wbs-icon-copy font-12 copy-public-key" @click="copyKey(contractName)" :title="$t('message.common.copy')"></i> </span>
                    ...
                    <div class="contract-info-list" v-show="abiFile">
                        <span class="contract-info-list-title" style="color: #0B8AEE">abi
                            <i class="wbs-icon-copy font-12 copy-public-key" @click="copyKey(abiFile)" :title="$t('message.common.copy')"></i>
                        </span>
                    ...
                    <div class="contract-info-list" style="border-bottom: 1px solid #e8e8e8" v-show="abiFile">
                        <span class="contract-info-list-title" style="color: #0B8AEE">bytecodeBin
                            <i class="wbs-icon-copy font-12 copy-public-key" @click="copyKey(bytecodeBin)" :title="$t('message.common.copy')"></i>
                        </span>
        ...
        <el-dialog v-dialogDrag :title="$t('message.contractIDE.sendTran')" :visible.sync="dialogVisible" width="500px" :before-close="sendClose" v-if="dialogVisible" center class="send-dialog">
        ...
        <el-dialog v-dialogDrag :title="$t('message.contractIDE.chooseAcc')" :visible.sync="dialogUser" width="500px" v-if="dialogUser" center class="send-dialog">
    ...
    methods: {
        ...
        compile: function () {
            ...
            try {
                ...
                this.errorInfo = this.$i18n.t('message.contractIDE.buildFail');
        ...
        changeOutput: function (obj) {
            if (JSON.stringify(obj) !== "{}") {
                if (obj.hasOwnProperty(this.contractName)) {
                    ...
                    this.successInfo = this.$i18n.t('message.contractIDE.buildSuc');
                    ...
                    this.$message({
                        type: "error",
                        message: this.$i18n.t('message.contractIDE.keepSame')
                    })
                    this.errorInfo = this.$i18n.t('message.contractIDE.buildFail');
                    this.compileShow = true;
                    this.loading = false;
            ...
            getDeployStatus(reqData)
                .then(res => {
                    ...
                    if (res.data.code === 0) {
                        ...
                        this.successInfo = this.$i18n.t('message.contractIDE.deploySuc');
                        this.abiFile = res.data.data.contractAbi;
                        this.bin = res.data.data.contractBin;
                        this.$message({
                            message: this.$i18n.t('message.contractIDE.conDeploySuc'),
                            type: "success"
                        });
                        this.data.contractAbi = this.abiFile;
                        this.data.contractBin = this.bin;
