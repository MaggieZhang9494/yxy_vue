 ...
 */
<template>
    <el-dialog v-dialogDrag :title="$t('message.transactionInfo.transactionContent')" :visible.sync="editorDialog" @close="modelClose" width="650px" top="10vh">
        <div v-if='!transationData' style="color:#e5e5e5" >{{$t('message.common.noData')}}</div>
                            ...
                            <el-table :data="inputData" v-if="inputData.length" style="display:inline-block;width:350px">
                                ...
                                <el-table-column prop="data" label="data" align="left" :show-overflow-tooltip="true">
                                    <template slot-scope="scope">
                                        <i class="wbs-icon-baocun font-12 copy-public-key" @click="copyPubilcKey(scope.row.data)" :title="$t('message.common.copy')"></i>
...
export default {
    ...
    data: function () {
        return {
            ...
            decodeData: "",
            showDecode: true,
            buttonTitle: this.$i18n.t('message.common.decode'),
    ...
    methods: {
        ...
        decode: function (val) {
            if (val.eventDataShow) {
                ...
                this.eventTitle = this.$i18n.t('message.common.decode')
            } else {
                this.$set(val, 'eventDataShow', true);
                this.eventTitle = this.$i18n.t('message.common.restore')
            }
