...
<template>
    <div class="send-wrapper">
        ...
        <div class="send-item">
            <span class="send-item-title">{{$t('message.contractIDE.contractName')}}:</span>
            <span>{{data.contractName}}</span>
        </div>
        <div class="send-item">
            <span class="send-item-title">{{$t('message.contractIDE.contractAddress')}}:</span>
            <el-input size="mini" v-model.trim="contractAddress" style="width: 240px;" :placeholder="$t('message.contractIDE.inputContractAddress')"></el-input>
            <el-tooltip class="item" effect="dark" :content="$t('message.contractList.leadContractAddress')" placement="top-start">
                <i class="el-icon-info"></i>
            </el-tooltip>
        </div>
        <div class="send-item" v-show="!constant">
            <span class="send-item-title">{{$t('message.common.user')}}:</span>
            <el-select size="mini" v-model="transation.userName" :placeholder="$t('message.common.chooseUser')" style="width:240px">
        ...
        <div class="send-item" style="padding-bottom: 10px;">
            <span class="send-item-title">{{$t('message.contractList.method')}}:</span>
            <el-select size="mini" v-model="transation.funcType" :placeholder="$t('message.contractList.methodType')" @change="changeType($event)" style="width:110px">
                <el-option label="function" :value="'function'"></el-option>
            </el-select>
            <el-select size="mini" v-model="transation.funcName" :placeholder="$t('message.contractList.methodName')" filterable v-show="funcList.length > 0" @change="changeFunc" style="width:125px">
        ...
        <div class="send-item" v-show="pramasData.length" style="line-height: 25px;">
            <span class="send-item-title" style="position: relative;top: 0px;">{{$t('message.contractList.param')}}:</span>
            <ul style="position: relative;top: -25px;">
                <li v-for="(item,index) in pramasData" style="margin-left:63px;">
                    <el-input size="mini" v-model="transation.funcValue[index]" style="width: 240px;" :placeholder="item.type">
                ...
                <p style="padding: 5px 0 0 28px;width:400px;"><i class="el-icon-info" style="padding-right: 4px;"></i>{{$t('message.contractList.parmaHint')}}</p>
            </ul>
        ...
        <div class="send-btn">
            ...
            <el-button type="primary" @click="submit('transation')" :disabled='buttonClick'>{{$t('message.common.affirm')}}</el-button>
        </div>
    </div>
</template>
<script>
                    ...
                    if (res.data.code === 0) {
                        ...
                        if (this.constant) {
                            ...
                            if (resData.statusOK) {
                                this.$message({
                                    type: "success",
                                    message: this.$i18n.t('message.contractList.transactionSuc')
                                });
                            } else {
                                this.$message({
                                    type: "success",
                                    message: this.$i18n.t('message.contractList.transactionFail')
                                });
                            }
                        }
                    } else {
