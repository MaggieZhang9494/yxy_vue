...
<template>
    <div>
        ...
        <v-content-head :headTitle="'message.menuName.nodeManagement'" :headTitlePath="{path:'/front'}" @changGroup="changGroup"></v-content-head>
        <div class="module-wrapper" style="height:auto;">
            <h3 style="padding: 20px 0 0 40px;">{{$t('message.nodeManagement.prepositionList')}}</h3>
            <div class="search-part" style="padding-top: 0px;position: relative;top:-25px;">
                <div class="search-part-left" v-if='!disabled'>
                    <el-button type="primary" class="search-part-left-btn" size="small" @click="createFront">{{$t('message.nodeManagement.addPreposition')}}</el-button>
                </div>
                <!-- <div class="search-part-right">
                    <el-input size="mini" placeholder="请输入前置编号" v-model="frontId" class="input-with-select">
            ...
            <div class="front-table">
                <el-table :data="frontData" class="search-table-content" v-loading="loading">
                    ...
                    <el-table-column fixed="right" :label="$t('message.common.operate')" width="100">
                        <template slot-scope="scope">
                            <el-button :disabled="disabled" :class="{'grayColor': disabled}" @click="deletedFront(scope.row)" type="text" size="small">{{$t('message.common.delete')}}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
        ...
        <div class="module-wrapper" style="margin-top: 10px;height:auto;">
            <div class="search-table">
                <h3 style="padding: 20px 0 8px 0;">
                    {{$t('message.nodeManagement.nodeList')}}
                    <el-tooltip effect="dark" :content="$t('message.nodeManagement.nodeExplain')" placement="top-start">
                ...
                <el-table :data="nodeData" class="search-table-content" v-loading="loadingNodes" >
                    ...
                    <el-table-column v-for="head in nodeHead" :label="head.name" :key="head.enName" show-overflow-tooltip >
                        <template slot-scope="scope">
                            ...
                            <template v-else>
                                <el-button :disabled="disabled" type="text" size="small" :style="{'color': disabled?'#666':''}" @click="modifyNodeType(scope.row)">{{$t('message.common.modify')}}</el-button>
                            </template>
                        </template>
                ...
                <el-dialog :title="$t('message.nodeManagement.modifyNodeType')" :visible.sync="modifyDialogVisible" width="387px" v-if="modifyDialogVisible" center>
    ...
    data() {
        return {
            ...
            frontHead: [
                {
                    enName: "frontId",
                    name: this.$i18n.t('message.nodeManagement.prepositionNum')
                },
                {
                    enName: "frontIp",
                    name: this.$i18n.t('message.nodeManagement.ip')
                },
                {
                    enName: "frontPort",
                    name: this.$i18n.t('message.nodeManagement.prepositionPort')
                },
                {
                    enName: "nodeId",
                    name: this.$i18n.t('message.nodeManagement.nodeId')
                },
                {
                    enName: "agency",
                    name: this.$i18n.t('message.nodeManagement.organization')
                },
                {
                    enName: "createTime",
                ...
                {
                    enName: "modifyTime",
                    name: this.$i18n.t('message.common.modifyTime')
                }
            ],
            nodeHead: [
                {
                ...
                {
                    enName: "nodeType",
                    name: this.$i18n.t('message.nodeManagement.nodeType'),
                    width: 180
                },
                {
                    enName: "blockNumber",
                    name: this.$i18n.t('message.blockInfo.blockHeight'),
                    width: 180
                },
                {
                    enName: "pbftView",
                    name: this.$i18n.t('message.nodeManagement.pbftView'),
                    width: 180
                },
                {
                    enName: "nodeActive",
                    name: this.$i18n.t('message.nodeManagement.status'),
                    width: 150
                },
                {
                    enName: "operate",
    ...
    methods: {
        ...
        dealZH_EN(){
            if(localStorage.getItem("internationalization") == 'zh-cn'){ /* 中文版本 */
                this.frontHead = this.frontHead.map(item=>{
                    if(item.enName=='nodeId'){//前置编号
                        item.width = '150px';
                    }else if(item.enName=='nodeType'){//ip
                        item.width = '150px';
                    }else if(item.enName=='blockNumber'){//前置端口
                        item.width = '150px';
                    }else if(item.enName=='pbftView'){//节点id
                        item.width = '150px';
                    }else if(item.enName=='nodeActive'){//所属机构
                        item.width = '150px';
                    }else{
                        item.width = '150px';
                    }
                ...
                this.nodeHead = this.nodeHead.map(item=>{
                    if(item.enName=='nodeId'){//节点id
                        item.width = '150px';
                    }else if(item.enName=='nodeType'){//节点类型
                        item.width = '180px';
                    }else if(item.enName=='blockNumber'){//块高
                        item.width = '180px';
                    }else if(item.enName=='pbftView'){//pbftView
                        item.width = '180px';
                    }else if(item.enName=='nodeActive'){//状态
                        item.width = '150px';
        ...
        getFrontTable() {
            ...
            getFronts(reqData)
                ...
                .catch(err => {
                    this.loading = false;
                    this.$message({
                        message: this.$i18n.t('message.common.searchFail'),
                        type: "error",
                        duration: 2000
                    });
                    this.$message.closeAll()
        ...
        nodesStatus(val) {
            ...
            switch (val) {
                case 1:
                    transString = this.$i18n.t('message.nodeManagement.run');
                    break;
                case 2:
                    transString = this.$i18n.t('message.nodeManagement.unusual');
                    break;
            }
            return transString;
        },
        nodeText(key) {
            ...
            switch (key) {
                case 'sealer':
                    str = this.$i18n.t('message.nodeManagement.nodeTypeConsensus');
                    break;
                case 'observer':
                    str = this.$i18n.t('message.nodeManagement.nodeTypeLook');
                    break;
                case 'remove':
                    str = this.$i18n.t('message.nodeManagement.nodeTypeFree');
                    break;
            }
            return str;
        },
        ...
        deleteNodes(val, type) {
            ...
            this.nodesDialogTitle = this.$i18n.t('message.nodeManagement.nodeDelete');
        },
        close(val) {
