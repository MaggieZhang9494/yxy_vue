 ...
 */
 <template>
    <div>
        <v-content-head :headTitle="title" :headTitlePath="{path:'/importStatisticsVVD'}"></v-content-head>
        <div class="module-wrapper">
            <el-menu :default-active="activeIndex" class="el-menu-statistics" mode="horizontal" @select="handleSelect" text-color="#000" active-text-color="#1585FF">
                <el-menu-item index="1">{{$t('message.statistics.importVVD')}}</el-menu-item>
            </el-menu>
                ...
                <div style="border:solid 1px #f1f1f1;height:100%;margin:20px;">
                    <div style="padding:20px 20px 10px 20px;">
						...
						<div>
							...
							<div class="search-item">
                                <span>{{$t('message.businessSearch.importCountryArea')}}：</span>
                                <el-select v-model="polCountry" size="mini" @change="changeNodes" :placeholder="$t('message.common.choose')" filterable>
                                    <el-option v-for="item in polCountryList" :key="item.value" :label="item.node" :value="item.value"></el-option>
                                </el-select>
                            </div>
							<div class="search-item">
                                <span>{{$t('message.businessSearch.tradingCountry')}}：</span>
                                <el-select v-model="tradeCountry" size="mini" @change="changeNodes" :placeholder="$t('message.common.choose')" filterable>
                                    <el-option v-for="item in tradeCountryList" :key="item.value" :label="item.node" :value="item.value"></el-option>
                                </el-select>
                            </div>
...
import { getBusinessKindList, getFilterDateList,getBarChartStatistics,getTransModeList,getSummaryModeList,getPolCountryList,getTradeCountryList } from "@/util/api";
import { format, numberFormat,formatData } from "@/util/util.js";
import errcode from "@/util/errcode";
import Bus from "@/bus"
export default {
    name: "importStatisticsVVD",
    components: {
    ...
    created(){
		...
		//获取运输方式
		_this.loadTransportMode();
		//获取启运国
		_this.loadPolCountry();
		//获取贸易国
		_this.loadTradeCountry();
		//获取汇总方式
    ...
    methods: {
        ...
        dealSearchParams(){
			...
			//启运国
            if(!_this.isNullData(_this.polCountry)){
                _this.searchParams.POL_COUNTRY = _this.polCountry;
            }else{
                delete _this.searchParams.POL_COUNTRY
			}
			//贸易国
            if(!_this.isNullData(_this.tradeCountry)){
                _this.searchParams.TRADE_COUNTRY = _this.tradeCountry;
            }else{
                delete _this.searchParams.TRADE_COUNTRY
            }
        ...
        loadPolCountry(){
            ...
            let reqQuery = {};
            getPolCountryList(reqData).then(res => {
                if (res.data.code === 0) {
                    this.polCountryList = res.data.data
                } else {
                    let message = res.data.message?res.data.message:this.errcode.errCode.isCN?this.errcode.errCode[res.data.code].cn:this.errcode.errCode[res.data.code].en;
                    this.$message({
                        type: "error",
        ...
        loadTradeCountry(){
            ...
            let reqQuery = {};
            getTradeCountryList(reqData).then(res => {
                if (res.data.code === 0) {
                    this.tradeCountryList = res.data.data
                } else {
                    let message = res.data.message?res.data.message:this.errcode.errCode.isCN?this.errcode.errCode[res.data.code].cn:this.errcode.errCode[res.data.code].en;
                    this.$message({
                        type: "error",
