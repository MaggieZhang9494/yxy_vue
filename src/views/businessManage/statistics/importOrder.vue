<template>
    <div>
        <v-content-head :headTitle="title" :headTitlePath="{path:'/importOrder'}"></v-content-head>
        <div class="module-wrapper">
            <el-menu :default-active="activeIndex" class="el-menu-statistics" mode="horizontal" @select="handleSelect" text-color="#000" active-text-color="#1585FF">
                <el-menu-item index="1">{{$t('message.statistics.importOrder')}}</el-menu-item>
            </el-menu>
                ...
                <div style="border:solid 1px #f1f1f1;height:100%;margin:20px;">
                    <div style="padding:20px 20px 10px 20px;">
                        <div >
							...
							<div class="search-item">
								<span>{{$t('message.businessSearch.entryCustoms')}}：</span>
								<el-select v-model="entryCustoms" size="mini" @change="changeEntryCustoms" multiple filterable clearable :placeholder="$t('message.common.choose')">
									<el-option
									v-for="item in entryCustomsList"
									:key="item.value"
									:label="item.node"
									:value="item.value">
									<span style="position: relative;float:left;left:-10px;width:20px;height:20px;">
                            ...
                            <!-- <div class="search-item">
                                ...
                                <el-select
                                    ...
                                    :placeholder="$t('message.common.choose')">
                                    <el-option
                                    v-for="item in entryCustomsList" :key="item.value" :label="item.node" :value="item.value">
                                    </el-option>
                                </el-select>
                            </div> -->
                        </div>
...
import { getBusinessKindList, getFilterDateList, getSummaryModeList,getBarChartStatistics,executeMultiReqest,getTransModeList,getEntryCustomsList } from "@/util/api";
...
export default {
    ...
    methods: {
        ...
        changGroup(val){
            //获取不同进境关别的订单统计数据
            this.getEntryCustomsStatisticsData();
        },
        //初始化列表查询参数
        initSearchParams(){
            let _this = this;
        ...
        dealSearchParams(){
			...
			//进境关别处理
            if(!_this.isNullData(_this.entryCustoms) && _this.entryCustoms.length>0){
				if (!_this.isSelectEntryAll) {
					_this.searchParams.JJKA = _this.entryCustoms.join(",");
				} else {
					delete _this.searchParams.JJKA
				}
            }else{
                delete _this.searchParams.JJKA
			}
        ...
        //获取进境关别
        loadEntryCustomsList(){
            let reqData = {
                ORDER_TYPE:this.searchParams.ORDER_TYPE
            };
            let reqQuery = {};
            getEntryCustomsList(reqData).then(res => {
                // this.loading = false;
                if (res.data.code === 0) {
					// this.entryCustomsList = res.data.data;
					
					//设置行业被选中状态，默认无行业被选中
                    this.entryCustomsList = res.data.data.map(item=>{
                        item.isSelect = false;
                        return item;
					});
                } else {
        ...
        exeMutilReq(){
            ...
            _this.loading = true;
            Promise.all([getFilterDateList({ORDER_TYPE:this.searchParams.ORDER_TYPE})]).then(res=>{
                if(res && res.length==1){
                    ...
                    //获取不同进境关别的订单统计数据
                    _this.getEntryCustomsStatisticsData();
                }
            }).catch(err => {
                _this.loading = false;
                let message = this.$i18n.t('message.common.systemError');
		...
		//处理已选中进境关别
		dealSelectEntryCustoms(){
			if(this.entryCustoms.length <= 0){
				return;
			}

			//如果已经选中了全部
			if (this.isSelectEntryAll) {
				let isHaveAll = this.entryCustoms.some(value => value === "");
				if (isHaveAll) {
					// 如果之前选中了全部，则变化后肯定不包含全部了，去除全部
					let indexAll = 0;
					for (let index = 0; index < this.entryCustoms.length; index++) {
						const element = this.entryCustoms[index];
						if(element === ""){
							indexAll = index;
							break;
						}
					}
					this.entryCustoms.splice(indexAll,1);

					this.isSelectEntryAll = false;
				} else {
					//本次取消全部
					this.entryCustoms = [];
					this.isSelectEntryAll = false;
				...
				let lastSelectValue = this.entryCustoms[this.entryCustoms.length-1];
				//如果最后选择是全部，则默认全选
				if(lastSelectValue === ""){
					this.entryCustoms = [];
					this.entryCustomsList.forEach(element => {
						this.entryCustoms.push(element.value);
					});
					this.isSelectEntryAll = true;
				}else{
					// 如果当前不包含全部，则判断是否其他的七个日期全选了
					if (this.entryCustoms.length === this.entryCustomsList.length - 1) {
						// 如果其他员工全选了，则也将当前置为全部
						this.entryCustoms.push("");
						this.isSelectEntryAll = true
					}
				}
			}
		},
		changeEntryCustoms(val){
			let _this = this;
			
			this.dealSelectEntryCustoms();

            //处理选中国家数据，更新选中状态
            _this.entryCustomsList = _this.entryCustomsList.map(element=>{
                let isSame = _this.entryCustoms.some(item=>{
                    return element.value == item;
                });
                element.isSelect = isSame;
                return element;
			})
			console.log("entryCustoms...",_this.entryCustoms);
		},
        changeNodes() {
            this.confirmParam();
        },
