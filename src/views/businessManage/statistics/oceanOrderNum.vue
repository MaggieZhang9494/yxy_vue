<template>
    <div>
        <v-content-head :headTitle="title" :headTitlePath="{path:'/oceanOrderNum'}"></v-content-head>
        <div class="module-wrapper">
            <el-menu :default-active="activeIndex" class="el-menu-statistics" mode="horizontal" @select="handleSelect" text-color="#000" active-text-color="#1585FF">
                <el-menu-item index="1">{{$t('message.statistics.oceanOrderNum')}}</el-menu-item>
            </el-menu>
                ...
                <div style="border:solid 1px #f1f1f1;height:100%;margin:20px;">
                    <div style="padding:20px 20px 10px 20px;">
                        <div >
                            ...
                            <div class="search-item">
                                <span>{{$t('message.statistics.entireType')}}：</span>
                                <el-select v-model="entireType" size="mini" @change="changeNodes" :placeholder="$t('message.common.choose')">
                                    <el-option v-for="item in entireTypeList" :key="item.value" :label="item.node" :value="item.value"></el-option>
                                </el-select>
                            </div>
                        </div>
                        <div>
...
import { getBusinessKindList, getFilterDateList, getSummaryModeList,getBarChartStatistics,executeMultiReqest,getTransModeList,getEntireTypeList } from "@/util/api";
...
export default {
    ...
    data() {
        return {
            ...
            searchParams:{
                ...
                TRADE_TOOL_TYPE_NAME:"水路运输",//运输方式，固定
                TRADE_CONT_TYPE:"",//整拼类型，非必传
                /*
                  D/W/M/Q/Y为单选项可选。
                  D: 每日daily （当日0点-24点）；
    ...
    methods: {
        ...
        dealSearchParams(){
            ...
            //整拼类型处理
            if(!_this.isNullData(_this.entireType)){
                _this.searchParams.TRADE_CONT_TYPE = _this.entireType;
            }else{
                delete _this.searchParams.TRADE_CONT_TYPE
			}
        ...
        loadEntireType(){
            ...
            let reqQuery = {};
            getEntireTypeList(reqData).then(res => {
                // this.loading = false;
                if (res.data.code === 0) {
                    _this.entireTypeList = res.data.data;
                } else {
                    let message = res.data.message?res.data.message:this.errcode.errCode.isCN?this.errcode.errCode[res.data.code].cn:this.errcode.errCode[res.data.code].en;
                    this.$message({
                        type: "error",
