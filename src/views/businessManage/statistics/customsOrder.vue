<template>
    <div>
        <v-content-head :headTitle="title" :headTitlePath="{path:'/customsOrder'}"></v-content-head>
        <div class="module-wrapper">
            <el-menu :default-active="activeIndex" class="el-menu-statistics" mode="horizontal" @select="handleSelect" text-color="#000" active-text-color="#1585FF">
                <el-menu-item index="1">{{$t('message.statistics.customs')}}</el-menu-item>
            </el-menu>
                ...
                <div style="border:solid 1px #f1f1f1;height:100%;margin:20px;">
                    <div style="padding:20px 20px 10px 20px;">
						...
						<div>
							...
							<div class="search-item">
                                <span>{{$t('message.common.finish')}}：</span>
                                <el-select v-model="finishType" size="mini" @change="changeNodes" :placeholder="$t('message.common.choose')">
                                    <el-option v-for="item in finishTypeList" :key="item.value" :label="item.node" :value="item.value"></el-option>
									<!-- <el-option v-if="isEN==false" v-for="item in finishTypeList" :key="item.value" :label="item.node" :value="item.value"></el-option>
									<el-option v-if="isEN" v-for="item in finishEnTypeList" :key="item.value" :label="item.node" :value="item.value"></el-option> -->
								</el-select>
                            </div>
...
import { getBusinessKindList, getFilterDateList, getSummaryModeList,getBarChartStatistics,executeMultiReqest,getTransModeList,getFinishTypeList } from "@/util/api";
...
export default {
    ...
    data() {
        return {
			...
			finishTypeList:[],
			// finishTypeList:[
			// 	{node: "海关放行", value: "海关放行"},
			// 	{node: "结关", value: "结关"},
			// ],
			// finishEnTypeList:[
			// 	{node: "Customs Release", value: "海关放行"},
			// 	{node: "Completed", value: "结关"},
			// ],
    ...
    methods: {
        ...
        dealSearchParams(){
			...
			//完成方式处理
            if(!_this.isNullData(_this.finishType)){
                _this.searchParams.finishType = _this.finishType;
            }else{
                delete _this.searchParams.finishType
            }
        ...
        loadFinishTypeList(){
            ...
            let reqQuery = {};
            getFinishTypeList(reqData).then(res => {
                if (res.data.code === 0) {
                    this.finishTypeList = res.data.data;
                } else {
                    let message = res.data.message?res.data.message:this.errcode.errCode.isCN?this.errcode.errCode[res.data.code].cn:this.errcode.errCode[res.data.code].en;
                    this.$message({
                        type: "error",
        ...
        exeMutilReq(){
            ...
            _this.loading = true;
            Promise.all([ getSummaryModeList({ORDER_TYPE:this.searchParams.ORDER_TYPE})]).then(res=>{
                if(res && res.length==1){
                    if (res[0].data.code === 0) {
                        _this.summaryWayList = res[0].data.data;
                    } else {
                        _this.loading = false;
