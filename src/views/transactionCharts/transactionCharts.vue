...
<template>
    <div>
        <v-content-head :headTitle="'message.menuName.userTransaction'" :headTitlePath="{path:'/transactionCharts'}" @changGroup="changGroup"></v-content-head>
        <div class="module-wrapper">
            <div class="more-search-table search-min-width">
                <div class="text-left">
                    <div class="text-left-label">
                        <span>{{$t('message.common.user')}}：</span>
                        <el-select size="mini" v-model="userName" @change="changeUserName" :placeholder="$t('message.common.choose')" clearable>
                    ...
                    <div class="text-left-label">
                        <span class="text-left-label-title">{{$t('message.userTransaction.time')}}：</span>
                        <el-date-picker size="mini" @change="changeDate" v-model="transDate" type="datetimerange" :picker-options="transPickerOptions" :range-separator="$t('message.common.to')" :start-placeholder="$t('message.common.startDate')" :end-placeholder="$t('message.common.endDate')" align="left" value-format="yyyy-MM-dd HH:mm:ss" class="date-select-32">
                        </el-date-picker>
                    </div>
                    <div class="text-left-label1">
                        <span class="text-left-label1-title">{{$t('message.userTransaction.interface')}}：</span>
                        <el-select size="mini" v-model="interfaceName" @change="changeInterfaceName" :placeholder="$t('message.common.choose')" clearable>
                ...
                shortcuts: [
                    ...
                    }, 
                    {
                        ...
                        onClick(picker) {
                            ...
                            const start = new Date();
                            let startTime = start.getTime();
                            start.setTime(startTime - 3600 * 1000 * 24 * 180);
                            picker.$emit("pick", [start, end]);
                        }
                    }
                ],
            ...
            monitorTransactionInfo(reqData, reqQurey)
                ...
                .catch(err => {
                    this.$message({
                        type: "error",
                        message:
                            this.errcode.errCode.isCN?this.errcode.errCode[err.data.code].cn:this.errcode.errCode[err.data.code].en ||
                            this.$i18n.t('message.userTransaction.getAccListFail')
                    });
