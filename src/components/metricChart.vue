...
<template>
    <div style="position: relative">
        ...
        <div class="noData" style="color: #2f4554" v-if="chartOption.data.contrastDataList.valueList.length === 0 && chartOption.data.contrastDataList.timestampList.length === 0 && chartOption.data.contrastDataList.contractDataShow">{{$t('message.common.noData')}}</div>
    </div>
</template>
    ...
    methods: {
        getChartData() {
            ...
            if (this.chartOption.data.contrastDataList.valueList.length > 0) {
                yContrastTitle = this.$i18n.t('message.nodeManagement.compareDayData');
            ...
            let option = {
                    ...
                    formatter: function(data) {
                        ...
                        if(data.length === 2 ){
                            if(data[0]['data'] && data[1]['data']){
                                ...
                                str = `
                                <span>${data[0]['name']}</span><br/>
                                <span>${data[0]['seriesName']}:${i18n.t('message.hostMonitor.noData')}</span><br/>
                                ...
                                <span>${data[0]['name']}</span><br/>
                                <span>${data[0]['seriesName']}:${data[0]['data']}</span><br/>
                                <span>${data[1]['seriesName']}:${i18n.t('message.hostMonitor.noData')}</span><br/>
                                `
                            }else {
                                str = `
                                <span>${data[0]['name']}</span><br/>
                                <span>${i18n.t('message.hostMonitor.noData')}</span><br/>
                                `
                            }
                ...
                legend: {
                    data: [this.$i18n.t('message.nodeManagement.showDayData'), yContrastTitle],
                    y: 35
                },
                dataZoom: [
                    {
                ...
                toolbox: {
                    ...
                    feature: {
                        dataZoom: {
                            yAxisIndex: "none",
                            title:{
                                zoom:this.$i18n.t('message.hostMonitor.zoomRegion'),
                                back:this.$i18n.t('message.hostMonitor.zoomRegionRestore'),
                            }
                        },
                        restore: {
                            show:true,
                            title:this.$i18n.t('message.hostMonitor.restore')
                        },
                        magicType: { 
                            show: this.showContrast ? true : false, 
                            type: ["stack", "tiled"],
                            title:{
                                stack:this.$i18n.t('message.hostMonitor.stack'),
                                tiled:this.$i18n.t('message.hostMonitor.tiled'),
                            }
                        }
                    }
                },
                ...
                series: [
                    {
                        name: this.$i18n.t('message.nodeManagement.showDayData'),
                        type: "line",
                        data: yList,
                        smooth: true
                    },
        ...
        formatNodesMetric(val){
            let num ;
            if(val === 0) {
                return str = this.$i18n.t('message.hostMonitor.noGetData')
            }
        }
    }
};
