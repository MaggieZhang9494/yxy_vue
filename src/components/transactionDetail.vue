        ...
        <el-tabs type="border-card" @tab-click="handleClick">
            <el-tab-pane label="input">
                <div>
                    ...
                    <div class="item">
                        <span class="label">
                            <el-tooltip class="item" effect="dark" :content="$t('message.unusualUser.nullHeyue')" placement="top-start">
                                    ...
                                    <el-table :data="inputData" v-if="inputData.length" style="display:inline-block;width:400px">
                                        ...
                                        <el-table-column prop="data" label="data" align="left" :show-overflow-tooltip="true">
                                            <template slot-scope="scope">
                                                <i class="wbs-icon-copy font-12 copy-public-key" @click="copyPubilcKey(scope.row.data)" :title="$t('message.common.copy')"></i>
            ...
            <el-tab-pane label="event" v-if="eventLog.length > 0" @click="decodeEventClick">
                <template v-if="unEvent">
                    <div  class="text-center">
                        {{$t('message.unusualUser.noAnalysis')}}
                    </div>
                </template>
                <template v-else>
...
export default {
    ...
    data: function () {
        return {
            ...
            funcData: "",
            showDecode: false,
            buttonTitle: this.$i18n.t('message.common.restore'),
            eventTitle: this.$i18n.t('message.common.restore'),
            abiType: [],
            methodId: "",
            inputData: [],
            eventLog: [],
    ...
    methods: {
                ...
                .then(res => {
                    if (res.data.code === 0) {
                        ...
                        if (res.data.data) {
                            ...
                            this.$message({
                                type: "error",
                                message: this.$i18n.t('message.unusualUser.noSearch')
                            });
                        }
                    } else {
                        this.$message({
        ...
        decode: function () {
            if (this.showDecode) {
                this.buttonTitle = this.$i18n.t('message.common.restore');
                this.showDecode = false;
            } else {
                this.buttonTitle = this.$i18n.t('message.common.decode');
        ...
        decodeEvent: function (eventData, data) {
            ...
            let list = data;
            list.eventTitle = this.$i18n.t('message.common.restore')
        ...
        decodeButtonEvent: function (num) {
            if (this.eventLog[num].eventDataShow) {
                ...
                this.$set(this.eventLog[num], 'eventTitle', this.$i18n.t('message.common.decode'))
                this.$set(this.eventLog, num, this.eventLog[num])
            } else {
                this.$set(this.eventLog[num], 'eventDataShow', true);
                this.$set(this.eventLog[num], 'eventTitle', this.$i18n.t('message.common.restore'));
                this.$set(this.eventLog, num, this.eventLog[num])
            }
        },
        //transactionDecode
